...
`tests/regression/sv_codegen_test.cpp` が読み込むCmソース断片。テーマ別のサブフォルダ（expr/ control/ process/ memory/ module/ verify/ errors/）に配置し、ファイル名はテスト名のsnake_case（例: `SignedConstantComparison` →`signed_constant_comparison.cm`）。
