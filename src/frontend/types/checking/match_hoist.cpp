...
//
// HIRは式形式matchを三項演算子チェーンへ脱糖し、scrutineeを
// アームごとにclone_hir_exprで複製する。関数呼び出しは複製すると
// 多重評価になるためクローン対象外で、従来はダミー値(0)へ
// フォールバックし誤った結果を返していた。
// 本パスは型チェック前にASTを書き換え、呼び出しを含むscrutineeを
// 文の直前の一時変数（auto推論）へ退避して単一評価を保証する。
//
...
class MatchHoister {
   ...
   private:
    ...
    // 文の実行時に無条件で1回評価される部分式から退避対象matchを収集する
    // （短絡評価の右辺・条件分岐の枝には入らない）
