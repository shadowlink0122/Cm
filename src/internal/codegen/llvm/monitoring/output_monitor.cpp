...
void OutputMonitor::begin_file(const std::string& filename) {
...
void OutputMonitor::write_data(size_t bytes) {
    ...
    if (info.total_bytes_written > max_file_size) {
        throw std::runtime_error(
            "出力サイズ超過: ファイル '" + current_file + "' が上限(" + format_size(max_file_size) +
            ")を超えました。現在のサイズ: " + format_size(info.total_bytes_written));
    }
    ...
    auto duration = std::chrono::duration_cast<std::chrono::seconds>(now - info.start_time).count();
...
void OutputMonitor::check_actual_file_size(const std::string& filename) {
    if (std::filesystem::exists(filename)) {
        ...
        if (actual_size > max_file_size) {
            throw std::runtime_error("出力サイズ超過: ファイル '" + filename + "' の実際のサイズ(" +
                                     format_size(actual_size) + ")が上限を超えています");
        }
    }
}

std::string OutputMonitor::format_size(size_t bytes) {
...
std::string OutputMonitor::get_statistics() const {
    ...
    for (const auto& [filename, info] : file_info) {
        auto duration =
            std::chrono::duration_cast<std::chrono::seconds>(info.last_write_time - info.start_time)
                .count();

        stats += "  " + filename + ": " + format_size(info.total_bytes_written) + " (" +
                 std::to_string(info.line_count) + " lines, " + std::to_string(duration) + "s)\n";
    }
