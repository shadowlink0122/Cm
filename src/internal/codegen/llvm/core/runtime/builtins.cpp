...
/// print・文字列・スライス・配列系ランタイム関数のシグネチャ宣言（該当しない場合はnullptr）
llvm::Function* MIRToLLVM::declareBuiltinRuntimeFunction(const std::string& name) {
    ...
    // コードポイント列挙（CmSlice*を返す）
    else if (name == "__builtin_string_chars") {
    ...
    // スライス操作関数
        ...
        auto func = module->getOrInsertFunction(name, funcType);
        return llvm::cast<llvm::Function>(func.getCallee());
    } else if (name == "cm_slice_pop_blob") {
        // void cm_slice_pop_blob(i8* slice) — blob要素popのlen減算（値はelement_ptr経由。W3）
        auto funcType = llvm::FunctionType::get(ctx.getVoidType(), {ctx.getPtrType()}, false);
        auto func = module->getOrInsertFunction(name, funcType);
        return llvm::cast<llvm::Function>(func.getCallee());
    ...
    // 汎用スライス要素アクセス（多次元配列用）
        ...
        auto func = module->getOrInsertFunction(name, funcType);
        return llvm::cast<llvm::Function>(func.getCallee());
    } else if (name == "cm_string_from_byte_slice") {
        // char* cm_string_from_byte_slice(void* slice)
        auto funcType = llvm::FunctionType::get(ctx.getPtrType(), {ctx.getPtrType()}, false);
        auto func = module->getOrInsertFunction(name, funcType);
        return llvm::cast<llvm::Function>(func.getCallee());
