...
// macro_rules! 定義全体を解析
std::unique_ptr<macro::MacroDefinition> MacroParser::parse_macro_rules(std::vector<Token>& tokens,
                                                                       size_t& pos) {
...
// マクロ名を解析
std::string MacroParser::parse_macro_name(const std::vector<Token>& tokens, size_t& pos) {
...
// マクロルールセットを解析
std::vector<macro::MacroRule> MacroParser::parse_macro_rules_body(const std::vector<Token>& tokens,
                                                                  size_t& pos) {
...
// 単一のマクロルールを解析
macro::MacroRule MacroParser::parse_single_rule(const std::vector<Token>& tokens, size_t& pos) {
...
// マクロパターンを解析
macro::MacroPattern MacroParser::parse_pattern(const std::vector<Token>& tokens, size_t& pos) {
...
// マクロトランスクライバーを解析
macro::MacroTranscriber MacroParser::parse_transcriber(const std::vector<Token>& tokens,
                                                       size_t& pos) {
...
// トークンツリーを解析
macro::TokenTree MacroParser::parse_token_tree(const std::vector<Token>& tokens, size_t& pos) {
    if (pos >= tokens.size()) {
...
std::unique_ptr<macro::DelimitedTokens> MacroParser::parse_delimited(
    const std::vector<Token>& tokens, size_t& pos, macro::DelimiterKind delimiter) {
    ...
    while (pos < tokens.size() && depth > 0) {
        if (is_delimiter_open(tokens[pos]) && get_delimiter_kind(tokens[pos]) == delimiter) {
            depth++;
...
std::optional<macro::MetaVariable> MacroParser::parse_metavar(const std::vector<Token>& tokens,
                                                              size_t& pos) {
    if (pos >= tokens.size() || tokens[pos].type != TokenType::Identifier) {
        return std::nullopt;
    }
...
macro::RepetitionOp MacroParser::get_repetition_op(const Token& token) const {
    ...
    if (token.value == "+")
        return macro::RepetitionOp::ONE_OR_MORE;
    if (token.value == "?")
        return macro::RepetitionOp::ZERO_OR_ONE;
    // デフォルト（エラー時）
    return macro::RepetitionOp::ZERO_OR_MORE;
}

// エラー処理
void MacroParser::expect_token(const std::vector<Token>& tokens, size_t& pos, TokenType expected,
...
void MacroParser::error(const std::string& message, const SourceLocation& location) {
    throw std::runtime_error(
        fmt::format("[MACRO_PARSER] ERROR at {}:{}: {}", location.line, location.column, message));
}
