    ...
    if (st.fields.empty()) {
        ...
        for (size_t i = 0; i < st.fields.size(); ++i) {
            ...
            if (field.type && field.type->kind == hir::TypeKind::Struct) {
                ...
                if (!exists) {
                    // ネスト構造体のHirStructから保持系の生成器で再帰生成する
                    // （モノモーフィゼーション後の手組み生成器は廃止済み。auto-impl第3段）
                    auto hs_it = hir_structs_by_name_.find(field.type->name);
                    if (hs_it != hir_structs_by_name_.end() && hs_it->second) {
                        generate_builtin_lt_operator(*hs_it->second);
                    }
                }
