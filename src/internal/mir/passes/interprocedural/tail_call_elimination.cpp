...
bool TailCallElimination::is_self_call(const MirFunction& func,
...
bool TailCallElimination::is_tail_position(const MirFunction& func,
                                           const MirTerminator::CallData& call_data) {
...
bool TailCallElimination::transform_to_loop(MirFunction& func, BasicBlock& call_block,
    ...
    // call_blockの終端子をループヘッダーへのGotoに変更
    call_block.terminator = MirTerminator::goto_block(loop_header_id);

    return true;
}

MirOperandPtr TailCallElimination::clone_operand(const MirOperand& src) {
