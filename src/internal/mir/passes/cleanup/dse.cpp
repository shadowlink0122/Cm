...
bool DeadStoreElimination::process_block(BasicBlock& block, const MirFunction& func) {
    bool changed = false;
...
void DeadStoreElimination::collect_uses(const MirStatement& stmt, std::unordered_set<LocalId>& used,
                                        bool& uses_deref) {
    if (stmt.kind == MirStatement::Assign) {
        auto& assign_data = std::get<MirStatement::AssignData>(stmt.data);
...
void DeadStoreElimination::collect_rvalue_uses(const MirRvalue& rvalue,
                                               std::unordered_set<LocalId>& used,
                                               bool& uses_deref) {
    switch (rvalue.kind) {
        case MirRvalue::Use: {
...
void DeadStoreElimination::collect_operand_uses(const MirOperand& op,
                                                std::unordered_set<LocalId>& used,
                                                bool& uses_deref) {
...
void DeadStoreElimination::collect_terminator_uses(const MirTerminator& term,
                                                   std::unordered_set<LocalId>& used,
                                                   bool& uses_deref) {
    switch (term.kind) {
...
bool DeadStoreElimination::has_side_effects(const MirRvalue& /* rvalue */) const {
    return false;
}
