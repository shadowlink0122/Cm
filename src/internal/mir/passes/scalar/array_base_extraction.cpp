...
bool ArrayBaseExtraction::process_block(MirFunction& func, BasicBlock& block) {
    bool changed = false;
...
bool ArrayBaseExtraction::has_multi_dim_index(const MirPlace& place) {
    int indexCount = 0;
...
std::vector<MirStatementPtr> ArrayBaseExtraction::transform_place(
    ...
    if (it != baseOffsetCache.end()) {
        ...
        castStmt->data = MirStatement::AssignData{MirPlace(outerIndexCast), std::move(castRvalue)};
        ...
        mulStmt->data = MirStatement::AssignData{MirPlace(baseOffsetLocal), std::move(mulRvalue)};
...
std::vector<MirStatementPtr> ArrayBaseExtraction::transform_rvalue(
...
std::vector<MirStatementPtr> ArrayBaseExtraction::transform_operand(
...
uint64_t ArrayBaseExtraction::get_array_stride(const hir::TypePtr& type, size_t numDimensions) {
    if (!type)
        return 0;
