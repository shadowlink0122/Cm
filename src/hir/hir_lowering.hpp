...
class HirLowering {
   ...
   private:
    ...
    HirDeclPtr lower_function(ast::FunctionDecl& func) {
        debug::hir::log(debug::hir::Id::NodeCreate, "function " + func.name, debug::Level::Trace);
    ...
    HirDeclPtr lower_struct(ast::StructDecl& st) {
        debug::hir::log(debug::hir::Id::NodeCreate, "struct " + st.name, debug::Level::Trace);
    ...
    HirStmtPtr lower_while(ast::WhileStmt& while_stmt) {
        ...
        not_cond->operand = lower_expr(*while_stmt.condition);
