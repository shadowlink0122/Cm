...
void cm_slice_delete(void* slice_ptr, int64_t index) {
    ...
    memmove(data + index * slice->elem_size,
            data + (index + 1) * slice->elem_size,
...
// filter_i64 (クロージャ版)
void* __builtin_array_filter_i64_closure(void* arr_ptr, int64_t size, void* fn_ptr, int64_t capture) {
    if (!arr_ptr || !fn_ptr || size <= 0) return NULL;
