...
// ランタイムヘルパーコードを出力
inline void emitRuntime(JSEmitter& emitter, const std::unordered_set<std::string>& used_helpers) {
    ...
    if (needs("__cm_str_slice")) {
        ...
        emitter.emitLine(
            "else if (end < 0) end = len + end + 1;");  // Recursive/Inclusive adjustment
