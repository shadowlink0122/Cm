...
// 段階（構文解析・型検査・IR変換・コード生成）の例外境界はrun_protectedの1実装に集約し、失敗した段階名付きで報告する。
...
// パイプライン段階の例外境界（run_build内で唯一のcatch-all実装）。
// ユーザーに行為可能な失敗は各段階が診断+終了コード（値あり）で返し、ここへ到達する例外は実装バグとして段階名付きの内部エラーで報告する。
// 段階本体は継続ならnullopt、終了なら終了コードを返す
template <typename Fn>
std::optional<int> run_protected(const char* stage, Fn&& fn) {
    try {
        return fn();
    } catch (const std::exception& e) {
        std::cerr << i18n::msgf(i18n::MsgId::CliInternalError, stage, e.what());
        return kExitFailure;
    }
}
...
int run_build(cli::Options& opts, const char* argv0) {
    ...
    if (auto rc = run_protected("parse", [&]() -> std::optional<int> {
    ...
    // ========== 型検査段階 ==========
    if (auto rc = run_protected("typecheck", [&]() -> std::optional<int> {
            // ========== Type Checker ==========
            if (opts.debug)
            ...
            checker.set_structural_array_lowering(
    ...
    mir::MirProgram mir;
    hir::HirProgram hir;
    if (auto rc = run_protected("lowering", [&]() -> std::optional<int> {
            // ========== HIR Lowering ==========
            if (opts.debug)
            ...
            // 非JSターゲットでasync/awaitが使用されている場合はエラー
            {
                ...
                bool is_sv_target = (opts.target == "sv" || opts.target == "verilog" ||
                                     opts.target == "systemverilog");
    ...
    // ========== バックエンドディスパッチ ==========
    // 従来この区画は例外境界を持たず、codegen層のthrowはmain.cppの最外周で「internal error (main)」になっていた。
    // 段階名を保つためここもrun_protectedで包む
    ctx.code = std::move(code);
    ctx.run_sv_sim = run_sv_sim;
    ctx.sv_hierarchy_submodules = std::move(sv_hierarchy_submodules);
    auto backend_rc = run_protected("codegen", [&]() -> std::optional<int> {
        ...
        if (!opts.sanitizers.empty()) {
            ...
            if (!unsupported.empty()) {
                std::string target_name =
                    is_sv || is_js ? (opts.target.empty() ? "js" : opts.target)
    ...
    return backend_rc.value_or(kExitSuccess);
}
