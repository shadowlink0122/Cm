...
bool FunctionInlining::process_function(
    MirFunction& caller, const std::unordered_map<std::string, const MirFunction*>& func_map) {
    bool changed = false;
...
bool FunctionInlining::process_block(
    MirFunction& caller, BlockId block_id,
...
bool FunctionInlining::should_inline(const MirFunction& callee) {
...
void FunctionInlining::perform_inlining(MirFunction& caller, BlockId call_block_id,
...
MirStatementPtr FunctionInlining::clone_statement(const MirStatement& src) {
...
MirTerminatorPtr FunctionInlining::clone_terminator(const MirTerminator& src) {
...
MirOperandPtr FunctionInlining::clone_operand(const MirOperand& src) {
...
MirPlace FunctionInlining::clone_place(const MirPlace& src) {
    return src;
}

MirRvaluePtr FunctionInlining::clone_rvalue(const MirRvalue& src) {
...
void FunctionInlining::remap_block(BasicBlock& block, LocalId local_offset,
...
void FunctionInlining::remap_statement(MirStatement& stmt, LocalId offset) {
...
void FunctionInlining::remap_terminator(MirTerminator& term, LocalId lo,
                                        const std::vector<BlockId>& bm) {
...
void FunctionInlining::remap_place(MirPlace& p, LocalId offset) {
    p.local += offset;
...
void FunctionInlining::remap_operand(MirOperand& op, LocalId offset) {
    if (auto* p = std::get_if<MirPlace>(&op.data))
        remap_place(*p, offset);
}

void FunctionInlining::remap_rvalue(MirRvalue& rv, LocalId offset) {
