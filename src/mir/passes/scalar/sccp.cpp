...
bool SparseConditionalConstantPropagation::run(MirFunction& func) {
...
void SparseConditionalConstantPropagation::rebuild_cfg(MirFunction& func) {
...
bool SparseConditionalConstantPropagation::same_type(const hir::TypePtr& a,
                                                     const hir::TypePtr& b) const {
    if (a == b) {
        return true;
    }
    if (!a || !b) {
    ...
    switch (a->kind) {
        ...
        case hir::TypeKind::Array:
            return a->array_size == b->array_size && same_type(a->element_type, b->element_type);
...
bool SparseConditionalConstantPropagation::equal_constant(const MirConstant& a,
                                                          const MirConstant& b) const {
...
bool SparseConditionalConstantPropagation::equal_value(const LatticeValue& a,
                                                       const LatticeValue& b) const {
    if (a.kind != b.kind) {
        return false;
    }
...
SparseConditionalConstantPropagation::LatticeValue SparseConditionalConstantPropagation::meet(
    const LatticeValue& a, const LatticeValue& b) const {
...
void SparseConditionalConstantPropagation::analyze(
    const MirFunction& func, std::vector<std::vector<LatticeValue>>& in_states,
    ...
    while (!worklist.empty() && iterations < max_iterations) {
        ...
        std::vector<BlockId> successors = compute_successors(func, *block, out_states[block_id]);
...
std::vector<SparseConditionalConstantPropagation::LatticeValue>
SparseConditionalConstantPropagation::merge_predecessors(
    const MirFunction& func, BlockId block_id,
    const std::vector<std::vector<LatticeValue>>& out_states, const std::vector<bool>& reachable) {
...
std::vector<SparseConditionalConstantPropagation::LatticeValue>
SparseConditionalConstantPropagation::transfer_block(const MirFunction& func,
...
std::vector<BlockId> SparseConditionalConstantPropagation::compute_successors(
    const MirFunction& func, const BasicBlock& block, const std::vector<LatticeValue>& state) {
...
bool SparseConditionalConstantPropagation::states_equal(const std::vector<LatticeValue>& a,
...
SparseConditionalConstantPropagation::LatticeValue
SparseConditionalConstantPropagation::eval_operand(const MirFunction& func,
...
SparseConditionalConstantPropagation::LatticeValue
SparseConditionalConstantPropagation::eval_rvalue(const MirFunction& func, const MirRvalue& rvalue,
                                                  const std::vector<LatticeValue>& state) {
    switch (rvalue.kind) {
        case MirRvalue::Use: {
...
bool SparseConditionalConstantPropagation::can_bind_constant(const MirFunction& func, LocalId local,
...
std::optional<MirConstant> SparseConditionalConstantPropagation::eval_unary_op(
...
bool SparseConditionalConstantPropagation::apply_constants(
    MirFunction& func, const std::vector<std::vector<LatticeValue>>& in_states) {
    bool changed = false;
...
bool SparseConditionalConstantPropagation::rewrite_rvalue(const MirFunction& func,
                                                          MirRvalue& rvalue,
                                                          const std::vector<LatticeValue>& state) {
    bool changed = false;
...
bool SparseConditionalConstantPropagation::rewrite_terminator(
    const MirFunction& func, MirTerminator& term, const std::vector<LatticeValue>& state) {
    bool changed = false;
...
bool SparseConditionalConstantPropagation::rewrite_operand(const MirFunction& func,
                                                           MirOperand& operand,
                                                           const std::vector<LatticeValue>& state) {
    if (operand.kind != MirOperand::Copy && operand.kind != MirOperand::Move) {
        return false;
...
bool SparseConditionalConstantPropagation::replace_with_constant(MirRvaluePtr& rvalue,
                                                                 const MirConstant& constant) {
    if (!rvalue) {
        return false;
    }
...
std::optional<BlockId> SparseConditionalConstantPropagation::simplify_switch(
    const MirFunction& func, const MirTerminator::SwitchIntData& switch_data,
    const std::vector<LatticeValue>& state) {
...
bool SparseConditionalConstantPropagation::simplify_cfg(MirFunction& func) {
    bool changed = false;
    for (auto& block : func.basic_blocks) {
...
bool SparseConditionalConstantPropagation::remove_unreachable_blocks(MirFunction& func) {
    rebuild_cfg(func);
