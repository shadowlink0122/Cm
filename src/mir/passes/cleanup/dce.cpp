...
bool DeadCodeElimination::run(MirFunction& func) {
    bool changed = false;
...
bool DeadCodeElimination::remove_unreachable_blocks(MirFunction& func) {
...
bool DeadCodeElimination::remove_dead_stores(MirFunction& func) {
    // 使用されているローカル変数を収集
...
bool DeadCodeElimination::remove_dead_statements(MirFunction& func) {
    bool changed = false;
...
void DeadCodeElimination::collect_used_locals(const MirFunction& func, std::set<LocalId>& used) {
...
void DeadCodeElimination::collect_used_locals_in_statement(const MirStatement& stmt,
...
void DeadCodeElimination::collect_used_locals_in_rvalue(const MirRvalue& rvalue,
                                                        std::set<LocalId>& used) {
    switch (rvalue.kind) {
        case MirRvalue::Use: {
...
void DeadCodeElimination::collect_used_locals_in_operand(const MirOperand& op,
                                                         std::set<LocalId>& used) {
...
void DeadCodeElimination::collect_used_locals_in_terminator(const MirTerminator& term,
                                                            std::set<LocalId>& used) {
    switch (term.kind) {
...
bool DeadCodeElimination::has_side_effects(const MirRvalue* rvalue) const {
    if (!rvalue)
        return false;
