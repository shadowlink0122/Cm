...
// MIRプログラムを文字列に変換
std::string MirPrinter::to_string(const MirProgram& program) {
    std::stringstream ss;
    print(program, ss);
    return ss.str();
}
...
// 安全な型文字列変換
std::string MirPrinter::safe_type_to_string(hir::TypePtr type, int depth) const {
    if (!type)
        return "?";
...
// 関数出力
void MirPrinter::print_function(const MirFunction& func, std::ostream& out) {
...
// 基本ブロック出力
void MirPrinter::print_block(const BasicBlock& block, const MirFunction& /* func */,
                             std::ostream& out) {
    out << "    bb" << block.id << ": {\n";
...
// 文出力
void MirPrinter::print_statement(const MirStatement& stmt, std::ostream& out) {
    switch (stmt.kind) {
...
// 終端命令出力
void MirPrinter::print_terminator(const MirTerminator& term, std::ostream& out) {
    switch (term.kind) {
...
// Place出力
void MirPrinter::print_place(const MirPlace& place, std::ostream& out) {
    out << "_" << place.local;
...
// Operand出力
void MirPrinter::print_operand(const MirOperand& op, std::ostream& out) {
    switch (op.kind) {
...
// Rvalue出力
void MirPrinter::print_rvalue(const MirRvalue& rv, std::ostream& out) {
    switch (rv.kind) {
        case MirRvalue::Use: {
...
// 定数出力
void MirPrinter::print_constant(const MirConstant& c, std::ostream& out) {
    out << "const ";
    std::visit(
        [&out](auto&& arg) {
...
// ヘルパー関数
std::string MirPrinter::binary_op_to_string(MirBinaryOp op) {
    switch (op) {
...
std::string MirPrinter::unary_op_to_string(MirUnaryOp op) {
    switch (op) {
        case MirUnaryOp::Neg:
            return "-";
        case MirUnaryOp::Not:
...
std::string MirPrinter::aggregate_kind_to_string(const AggregateKind& kind) {
    switch (kind.type) {
...
std::string MirPrinter::escape_string(const std::string& s) {
    std::string result;
    for (char c : s) {
        switch (c) {
            case '\n':
