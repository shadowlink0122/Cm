...
inline void OptimizationPass::collect_used_locals(const MirFunction& func, std::set<LocalId>& used) {
    // この実装はDeadCodeEliminationから移動
    // 実装の詳細は各最適化パスで必要に応じてオーバーライド
}

inline std::optional<MirConstant> OptimizationPass::eval_binary_op(
    MirBinaryOp op,
        ...
        for (size_t i = 0; i < func.basic_blocks.size(); ++i) {
            ...
            if (block->statements.empty() &&
                ...
                for (auto& other_block : func.basic_blocks) {
                    if (!other_block || other_block == block) continue;
