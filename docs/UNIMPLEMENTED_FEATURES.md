...
| v0.14.0 | ✅ 完了 | JSバックエンド（通過率87%）、演算子オーバーロード、インラインユニオン型、UEFIサポート |
...
| JSバックエンド改善 | 🔧 部分実装 | ポインタ非依存セマンティクス（現在通過率87%） |
...
### v0.14.0 完了機能
- JSバックエンド（Node.js/ブラウザ向けJS生成、通過率87%）
- 演算子オーバーロード改善（`impl T { operator ... }`、複合代入演算子自動対応）
- インラインユニオン型（`int | null`構文）
- ベアメタル/UEFIサポート（`--target=uefi`）
- プラットフォームディレクティブ（`//! platform:`）
- VSCode拡張機能（構文ハイライト、TypeScript移行、ESLint/Prettier）
- Enum Associated DataのJSサポート
- ジェネリクスコンストラクタ/デストラクタのJSサポート
- 再帰的デストラクタインジェクションの改善
...
- **docs/releases/v0.14.0.md**: v0.14.0リリースノート
