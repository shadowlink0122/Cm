...
### 6.3 C/C++スタイルの単一宣言（v0.17.1）

型定義と同時に変数/フィールドを宣言できる（C/C++の `struct {...} x;` と同じ方式）。
匿名struct/enumは宣言子必須で、内部的には先頭宣言子から `__anon_<宣言子>` 名を合成した通常の型として扱われる。
...
struct Named {
    int v;
} g1, g2;           // 型Namedの定義とグローバルg1,g2の宣言

// struct本体内: 宣言子はフィールドになる
struct Outer {
    struct Inner {
        int mem;
    } a, b;         // 型Outer::Innerの定義とフィールドa,b
    enum { FAST, SLOW } mode;
};
```

- 末尾`;`省略構文（`struct X {}`）の直後に来る通常宣言（`P make()` 等）と区別するため、宣言子は「識別子の直後が `;` か `,`」の場合のみ受理される
- 宣言子への初期化子（`struct X {} g = ...;`）は未対応（別文 `X g = ...;` で書く）
- enum本体内（値スロットのみの文脈）では宣言子は使えない

**制約：**
- ジェネリックstruct/enumの本体にはネスト型を宣言できない
- ネスト型宣言自身にジェネリックパラメータは付けられず、ジェネリック型宣言に宣言子は付けられない
- extern struct内のネスト型・宣言子なしの匿名struct/enumは未対応
...
`__`（連続する2つのアンダースコア）を含む識別子はコンパイラ予約であり、ユーザ宣言（struct/enum/interface/typedef/関数/メソッド/変数/定数/フィールド/enumメンバ/引数/宣言子）には使用できない（構文エラー）。
予約の理由は、コンパイラ内部のマングル名（`Type__method`）・匿名型の合成名（`__anon_<宣言子>`）・SV出力の写像名（`Outer::Inner`→`Outer__Inner`）・組み込みシンボル（`__println__` 等）との衝突を構文レベルで排除するためである。

```cm
struct Box__int { int raw; }  // エラー: Identifiers containing '__' are reserved for the compiler
int x__y = 1;                 // エラー: 同上
int _helper() { ... }         // OK: 単一アンダースコアは自由
```

**例外（検査対象外）:**
- extern宣言・`use`（FFI）ブロック内の宣言は外部シンボル名（`__errno_location` 等）を写すため対象外
- `__asm__`・`__sizeof__` 等の組み込みキーワード、`#ifdef __x86_64__` 等のプリプロセッサシンボルの参照は宣言ではないため対象外
- 既存の識別子の参照（`Outer::__anon_mode::SLOW` のような合成名の参照）は可能
...
- 2026-08-14: 6.3節C/C++スタイルの単一宣言（匿名struct/enum・宣言子）を追加（v0.17.1）
- 2026-08-14: 識別子の予約（`__`を含む宣言名の禁止）を追加（v0.17.1）
