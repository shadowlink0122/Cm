...
1. 第1段（領域2）: `lower_place` 統合。W2再発クラスの根絶が最優先で、既存テスト（nested_slice_write等）が回帰網になる。
2. 第2段（領域4）: スライスディスパッチ表化。機械的で低リスク。
3. 第3段（領域3）: 期待型パラメータの正式化。型検査APIの変更を伴うため独立ブランチで実施。
4. 第4段（領域1）: 補間のパース時脱糖。削除量が最大だが、第1〜3段の統合APIが揃ってから行うと差分が最小になる。

各段でmake test全スイートを完走させ、削除した経路のテスト（interp_chain・push_array_literal等）が新経路で通ることを確認する。
