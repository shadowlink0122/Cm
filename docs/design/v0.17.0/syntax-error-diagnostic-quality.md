...
第6ラウンドの全カテゴリで繰り返し観測された診断品質の横断課題。X5（syntax-error-position-and-token-display）で意味解析エラーの行番号は改善されたが、パーサ/プリプロセッサ段のエラーが取り残されている。個別バグ（R1〜R13）の再現時、原因特定を一貫して妨げていた。
