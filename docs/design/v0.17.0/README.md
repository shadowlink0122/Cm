...
このうちレイアウト計算の一元化は実装完了し、[archive/v0.17.0/layout-query-unification.md](../../archive/v0.17.0/layout-query-unification.md) へ移動した（スライス格納/配列実ストライドの2意味論API・MIR/LLVM共有コア・unitテスト固定）。
...
型解決とチェーンloweringの単純化（type-resolution-simplification.md、補間フォールバック完全削除の監視期間のみ残）と、簡素化提案8本（compiler-architecture-restructure.md他）が未実装の構造的リファクタリング提案として本ディレクトリに残っている。
