...
4. **for-inプロトコル検証**: iter発見時にhas_next(): boolとnext()の存在・シグネチャを検査し、欠如は専用診断で停止する（forin-iterator-protocol-checks.mdの修正をこのAPIの上で行う）。
...
- 第1段（即効・低リスク）: throw4件の診断化（Q4修正）とfor-inプロトコル検証（Q1修正）。統一APIを待たずに実施できる。
