...
| # | 領域 | 所見 | 状態 |
|---|------|------|------|
| C14 | コンパイル時間 | 固定長配列を持つ構造体の代入・値渡しがO2で二次爆発（`int[16384]`フィールドで24秒/6.4GB、コピーがmemcpyでなく全要素SSA展開） | Phase 1・Phase 4実装済み（Phase 1: `assign.cpp`の構造体コピーで128バイト以上の集約を`CreateMemCpy`化。Phase 4: 16バイト超構造体の戻り値をsret（先頭の隠し出力ポインタ）へ変換し、`int[16384]`戻り値のO2コンパイルが実測60秒超タイムアウト→0.6秒。あわせて`isSmallStruct`のArrayフィールド8バイト誤計算をDataLayout判定へ修正、Bug#45引数個数チェックのsret非対応を+1補正、16バイト超構造体のポインタ渡し引数が呼び出し先の変更で呼び出し元へ波及する既存の値セマンティクスバグをbyval相当のエントリコピーで修正（selfはMIR型Pointerのため従来どおり参照渡し、jsのH3値渡しと整合）。除外条件: extern関数=FFI ABI互換、アドレス取得された関数=間接呼び出しのシグネチャ追跡不能、インターフェイス値=fat pointer16バイト） |
...
- **Phase 4（C14）**: 大きな構造体の関数引数・戻り値に`byval`/`sret`属性を導入し、集約渡しを第一級値経由から外す。
