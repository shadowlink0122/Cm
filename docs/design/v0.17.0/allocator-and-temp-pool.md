...
| # | 領域 | 所見 | 状態 |
|---|------|------|------|
| H11 | メモリ | wasmアロケータは解放不可能な単調バンプ（`free`/`realloc`が旧ブロックを放棄）で長時間実行プログラムは総確保量に比例してメモリ増加 | 未着手 |
| M14 | メモリ | `CmTempStringPool`は完全なデッドインフラ（呼び出しゼロ）、アロケータ差し替え（`cm_set_allocator`）はCmから到達不能で`std::mem`/`Vector`はlibc mallocを直呼び | 未着手 |
