...
| # | 領域 | 所見 | 状態 |
|---|------|------|------|
| M18 | バックエンド | SVバックエンドで`println`が無診断で捨てられ空moduleが生成される（合成不能文のドロップ警告なし） | 未着手 |
| L1 | 識別子 | グローバル変数のみidサフィックス無しで`sanitizeIdentifier`の多対一縮退衝突がありうる | 未着手 |
| L2 | 型システム | 文字列補間`{}`が内側の引用符・波括弧を誤パース（`{m.get(Key{name:"x"})}`が壊れる） | 未着手 |
| L3 | 型システム | `var`が推論キーワードでなく未解決型名として扱われ紛らわしいエラーになる（`auto`は動作する） | 未着手 |
| L5 | 言語 | fmtはインデント正規化中心（演算子・波括弧の空白は不統一） | 未着手 |
| L6 | 言語 | LSP・パッケージマネージャは未実装、`#[test]`のassertは`assert_eq`が無い | 未着手 |
| L7 | 言語 | 並行処理はnativeバックエンド専用でチャネルはint64スカラのみ | 未着手 |
