...
- 専用の関数ポインタ配列型（`int*(int,int)[N]`）はコード生成が未対応（JITでシンボル未解決）のため本変更の対象外とする。ただし関数ポインタを `void*` 配列に格納することは可能で、取り出して関数ポインタ型へ `as` キャストすれば呼び出せる（テスト `arrays/element-types/function_pointer`）。
