...
| 並行アサーション`assert property` | ✅ | `sv_assert_property`+時相演算子（$pastシフト形出力・verilator検証） → [SV-N7](../../../archive/v0.17.0/sv/concurrent-assertions-sva.md)（実装済み） |
...
| [SV-N7](../../../archive/v0.17.0/sv/concurrent-assertions-sva.md) | 並行アサーション（SVA） | Medium | 検証機能（実装済み） |
