...
| # | 領域 | 所見 | 状態 |
|---|------|------|------|
| H7 | 言語 | モジュールの`export`が自由関数に対して未強制で、非export関数を他モジュールからimportして呼べる（カプセル化が機能しない） | 段階2実装済み（選択importで明示指定された非export自由関数へ警告を出す。破壊的変更回避のためcheck/lintと`--force-check`/`--strict`コンパイル時のみ発火し、通常コンパイルは従来どおり。検出はモジュール元ソースのトップレベル走査で、`export {名前列挙};`形式・ジェネリック接頭辞`<T>`対応、型定義（struct/enum/const/typedef）の透過は仕様として維持。tests/common/modules・advanced_modules・libs/stdの全数スキャンで既存コードへの誤警告ゼロを確認。エラー化と段階4（複製ヘルパーの非修飾公開の抑止）は将来バージョンで実施。段階4の実装知見（v0.17.0で試行・撤回）: 複製ヘルパーの__cm_priv_改名方式は、export情報の消費箇所が3系統（normalize_export_blocksの複数行リスト正規化・process_export_syntaxのリスト処理と定義再配置・remove_export_keywordsの除去）に分散しており、選択importの経路ではremove適用済みソースが再度flowするため「非exportヘルパー」を安全に分類できる単一地点が存在しない。実施にはexportメタデータをソース文字列と分離して持ち回るパイプライン再設計（正規化時に収集した公開名集合を展開の最後まで伝搬する）が前提となる） |
