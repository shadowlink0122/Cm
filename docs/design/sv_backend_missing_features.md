...
>   キャスト経由でも括弧情報が失われない
>
> **残（Phase 2b・小）**: ternary変換（if/else単一代入→ ? : ）と
> else-if正規化（end else begin + if → end else if）の2つの行構造変換は
> テキストベースのまま。いずれも優先順位非依存の行パターン変換であり
> リスクは低いが、将来SVStmtノード化する場合はここも置換する。

**現状（Phase 1〜3完了）**: 式のツリー化・テンポラリの構造的インライン展開・
ラッチ推論のMIR解析化まで完了。生成SVからテンポラリ変数の中間代入が消え、
出力が大幅に簡潔になった（例: while_true のループ本体が5行→3行）。

**難易度**: 低（残りは行パターン変換2つのみ）
