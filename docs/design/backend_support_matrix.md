...
| matchのバインディング変数（`Option::Some(v)` の `v` 等）が型チェッカで関数スコープへ漏れる | 後続の同名変数宣言の型が誤解決されることがある（別名を使えば回避可能。実行時の値は正しい） | TCのcheck_match_patternがスコープpush/popなしでdefineしているため。修正は別途 |
| 補間内のResult/Optionメソッド呼び出し（`{o.unwrap_or(-1)}` 等）が未解決シンボルになる | 変数に受けてから補間する | ミニパイプラインの型（__TaggedUnion_名）がHIRメソッド脱糖の判定と不一致のため。修正は別途 |
