...
### 問題2: 学生管理
学生の情報（名前、年齢、成績）を管理する構造体を作成してください。
...
impl Student {
    self(string name, int age, int score) {
        this.name = name;
        this.age = age;
        this.score = score;
    }
...
int main() {
    Student[3] students;
    students[0] = Student("Alice", 20, 85);
    students[1] = Student("Bob", 21, 92);
    students[2] = Student("Charlie", 19, 78);
...
✅ コンストラクタが実装できる  
✅ ネストした構造体が使える  
⏭️ 次は [Enum型](enums.md) を学びましょう

## 関連リンク

- [インターフェース](interfaces.md) - メソッド定義
- [ジェネリクス](generics.md) - ジェネリック構造体
- [with自動実装](../advanced/with-keyword.md) - 自動トレイト実装
