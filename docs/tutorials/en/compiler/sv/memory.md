...
Array initializers are emitted as an `initial` block. FPGA synthesis tools treat them as ROM/RAM initial contents; simulators apply them at time 0:
...
Emits `$readmemh` to load contents from a hex file, so large ROMs no longer need giant case statements:
