...
アリーナは既定で最大48ノード（JSON値1つにつき1ノード）。この上限は全バックエンドが既定の最適化設定でコンパイルできるようにするため。LLVM（native/jit）バックエンドは巨大なグローバル集約があると最適化・コード生成コストが急増する。上限を超えると `json_parse` は `-1` を返す。js/tsにはこの制約が無いので、必要に応じて大きなアリーナを使える。
