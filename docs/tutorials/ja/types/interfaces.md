...
同じ構造体の `impl` ブロック内でのみ使用できる補助メソッドを定義できます（C++のprivateと同様に構造体単位の可視性で、同じ構造体の別インターフェース実装からも呼び出せます）。これはインターフェースのシグネチャには含まれませんが、共通のロジックをまとめるのに便利です。
...
impl MyCalc for Calculator {
    // 同じ構造体の impl ブロック内でのみ呼び出し可能（外部からはエラー）
