...
> `null` を変種に持つユニオン（`int | null`）は `null as NU` で構築し `n is null` で判別できます。固定長配列をスライス変種（`int[]`）へ格納すると自動でスライスへ実体化されます（いずれもv0.17.0で修正）。

### ユニオンの等値比較（v0.17.0で修正）

ユニオン同士・ユニオンと変種値の `==`/`!=` は、**タグ一致＋アクティブな変種のペイロード比較**で判定されます（文字列変種は内容比較）:
...
> **v0.17.0より前の注意:** 従来は生表現の比較に落ち、native/JITはタグのみ比較（`1 == 2` がtrue）、JSは参照比較（`1 == 1` がfalse）と全バックエンドで誤値でした。

ユニオンはジェネリック構造体の型引数にも渡せます（`Box<IU>`）。`#[derive(Eq)]`付きジェネリック構造体の特殊化もv0.17.0から正常動作し、等値は同じタグ+ペイロード比較で判定されます（詳細は[自動実装（with / #[derive]）](../advanced/with-keyword.html)を参照）。
