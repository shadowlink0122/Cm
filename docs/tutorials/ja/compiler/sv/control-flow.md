...
ループからの脱出は、名前付きブロックへの `disable`（Verilog-1995互換）として出力されます。SV-2005の `break` キーワードは古いIcarus Verilog（v11以前）や一部の合成ツールが未対応のため使用しません:
...
初期値・境界・増分がすべて定数のループは、SVターゲットのコンパイル時に直列のブロック列へ**静的展開**され、生成SVに `while` が残りません（合成ツールは動的whileを展開できないため）:
...
- `while (true)` + `break` のような無条件ヘッダのループも、名前付きブロック + `disable` で再構成されます（v0.15.1で対応済み）
...
Cmソースの式の構造（括弧・評価順序）は生成SVでも保持されます。SVでは `==` が `&` より優先されるため、括弧が失われると意味が変わりますが、コンパイラが必要な括弧を必ず出力します:
...
## don't careビットのマッチ（v0.16.0）

2進リテラルの `?` は「そのビットを比較しない」don't careです。命令デコーダのようなビットパターン分岐を簡潔に書けます:

```cm
match (op) {
    0b1?00 => { kind = 1; }   // 1x00 にマッチ（bit2は不問）
    0b0?1? => { kind = 2; }
    _ => { kind = 0; }
}
```
