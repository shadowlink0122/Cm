...
### コンパイル時に拒否される機能（v0.17.0で強制を強化）

no_std制約はコンパイル時に検査され、違反はエラーになります:

- **OS依存関数の直接呼び出し**: `println`・`malloc`・ファイルI/O・pthread等
- **禁止関数のアドレス取得**: `&putchar` のような関数ポインタ経由の間接呼び出しも回避手段になりません
- **ヒープ確保を伴う言語機能**: 文字列連結（`a + b`）・数値→文字列変換・文字列補間の整形・動的スライス操作・配列高階メソッド（いずれも内部でヒープ確保するため）

浮動小数点は、UEFI（x86_64 ABIはSSE有効）では使用できますが、`--target=baremetal-x86`（SSE無効）では専用診断で拒否されます。
