...
曖昧性のない二項演算子（`=` 複合代入 `==` `!=` `<=` `>=` `&&` `||` `=>` `<<=` `>>=`）の前後へ不足している空白を追加します。
既存の空白は削らないため、意図的な桁揃えは保持されます。
...
単独の `+ - * / % < > & ^` は単項演算子・ジェネリクス（`<T>`）・指数表記（`1e+5`）と区別できないため対象外です。
`operator` 宣言行は宣言スタイル（`operator bool ==(T other)`）を保持します。
