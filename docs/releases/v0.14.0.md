...
v0.14.0は、**JavaScriptバックエンドの大規模改善**、**演算子オーバーロードの設計改善**、**Enumの関連データ(Associated Data)修正**、**Tagged Unionの構造体ペイロード修正**を含むリリースです。JSバックエンドのテスト通過率が55%から77%に向上し、演算子オーバーロードでは`impl T`構文と複合代入演算子(`+=`等)をサポートしました。
