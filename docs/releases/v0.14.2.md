...
v0.14.2は**Tagged Union（Result/Option/enum）のペイロード抽出に関する重大バグ2件の修正**、**ASM過剰volatile生成の修正**、**Dead Function Elimination（DFE）の追加**を含むパッチリリースです。Cosmo Linux（Cm言語製カーネル）開発で発見されたコンパイラバグを修正し、ベアメタル環境でのResult型エラーハンドリングを安全に使用可能にしました。
...
> **影響**: この修正により`Result<T, E>`でT==E（例: `Result<long, long>`）が正しく動作。Cosmo Linuxの全レイヤーでResult型エラーハンドリングが使用可能に。
