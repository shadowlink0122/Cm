...
v0.13.0で確立した標準ライブラリ基盤を、本番利用可能な並行処理・ネットワーキング基盤へ進化させる。すべての外部モジュール（OS依存機能）を統一`Result<T, E>`型でラップし、安全なエラーハンドリングを保証する。
...
Result型はユーザー定義enum（`tests/test_programs/result/builtin_result_test.cm`等）として使用可能。コンパイラ組み込みではないため、各モジュールで再定義が必要。
