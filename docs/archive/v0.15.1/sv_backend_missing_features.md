...
> **✅ 2026-07-05 一部実装済み（UNUSED/UNDRIVEN削除・strictモード追加）**:
> 全SVテスト197ファイル+CmCPU実デザイン3種で lint_off なしのVerilator警告を
> 実測した結果:
>
> | カテゴリ | 警告数（スイート） | 実デザイン | 対応 |
> |---|---|---|---|
> | UNUSED | 0 | 0 | **抑止を削除** |
> | UNDRIVEN | 0 | 0 | **抑止を削除** |
> | WIDTHEXPAND | 56 | 1,761 | 既定で抑止継続 |
> | WIDTHTRUNC | 33 | 99 | 既定で抑止継続 |
>
> UNUSED/UNDRIVEN は式ツリー化による未使用テンポラリ宣言の完全除去で
> 警告ゼロになったため、抑止コメントを出力しないようにした。
> `--sv-strict-lint` オプションを新設し、指定時はWIDTH系も含め
