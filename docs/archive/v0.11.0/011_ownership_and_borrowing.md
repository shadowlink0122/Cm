...
Cm言語はRust風の所有権・借用システムを採用しますが、**mutキーワードは使用しません**。代わりに`const`キーワードで不変性を明示する方式を採用します。

## 基本原則

### 1. const-by-default（const優先）

変数が一度も変更されない場合、`const`を付けるべきです。付いていない場合は**警告**（将来的にはエラー）となります。
...
- [リファクタリング提案](refactoring_proposal.html) - データフロー解析ベースの借用チェッカー
- [型システム](type_system.html) - 型チェックの詳細
