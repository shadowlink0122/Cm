...
`impl`メソッドをポインタ経由（`ptr->method()`）で呼んだ場合、メソッド内での`self`フィールド変更が元の構造体に書き戻されない。
...
### 症状
構造体のフィールドが6個以上ある場合、implメソッドからフィールドインデックス5以上にアクセスするとLLVM verification errorが発生する。
...
### 症状
問題2を回避するためにフィールド数を5個以下にパックしても、implメソッドのself書き戻しで`load`命令のオペランド型不整合が発生する。
