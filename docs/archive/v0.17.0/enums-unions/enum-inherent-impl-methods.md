...
既知の制約: 値enumメソッド内の`self`再代入はコピーに閉じる（値意味論。チュートリアルに明記）。enum正規化の構造的遅延・メソッド表キーの正準化は[method-resolution-unification.md](../type-system/method-resolution-unification.md)が引き続き扱う。
