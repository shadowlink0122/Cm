...
| # | 領域 | 所見 | 状態 |
|---|------|------|------|
| H10 | スライス | チェーンレシーバ（`m[0].push(x)`・`make().len()`）が黙って欠落またはリンクエラー（レシーバ解決がVarRef/Memberのみ） | 実装済み（第1-2段+第4段: レシーバ解決を共通ヘルパ`resolve_receiver_place`へ統合しpush/len/capの黙殺を診断化、呼び出し戻り値レシーバは一時実体化。第3段: スライス要素の添字レシーバは`cm_slice_get_subslice_ref`（格納中の内側ヘッダへの参照、コピーしない）で場所化し、push/pop/delete/clear/len/capの変異が格納中のヘッダへ反映される。第5段: `g.cells[i].push(v)`混合チェーンはget_member_placeのフィールド場所→添字レシーバ連結で解決。多重添字読み`rows[0][1]`もget_subslice連鎖への還元で対応。jsはCopyPropagationの集約コピー伝播を抑止して深いクローン意味論との不整合を解消。固定長配列要素は従来どおりindexプロジェクション、構造体blob要素のレシーバは場所を持てないため診断で停止） |
