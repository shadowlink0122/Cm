...
レイヤー別レビュー追補（ユニオン・文字列要素の配列/スライス整合性調査）で検出。最小再現は `.tmp/bughunt4/z/z06_struct_with_string_array.cm`（native/jit/wasm比較）。

## 解決記録（2026-08-05）

- コミット単位のバイセクトで解決コミットを特定した: Y4（c5137702）時点ではwasmの`is`判定がfalse、Y1〜Y3（9d2f2e98: ユニオン構築の消費サイト統一）適用後にtrueとなり、現ブランチでも再現しない。
- 真因の確定: 本文書の見立て（オフセット計算のポインタ幅8仮定）は誤りで、実際はY1（構造体リテラルのユニオンフィールドがタグ未構築でペイロード直書き）だった。nativeで正しく見えていたのはメモリレイアウトの偶然（タグスロットの残留値が変種タグに一致）で、wasmはレイアウト差により不一致が露見していた。`coerce_to_union`によるターゲット非依存のタグ書き込みへ統一されたことで解消した。
- 検証: 本文書のテスト計画どおり、フィールド位置（先頭/中間/末尾）×前置フィールド型（string[2]・int・24バイト構造体）×`is`/`as`ペイロード/フィールド再代入のマトリクスを`tests/common/types/union_field_tag_layout.cm`として追加し、native/jit/wasm/jsの4系一致を確認した。全スイート通過。
- 教訓: バックエンド分裂の見立ては書き込み側と読み出し側の両方を疑うこと。「片方のバックエンドだけ正しい」場合でも、正しく見える側が偶然（未定義値の一致）である可能性がある。
