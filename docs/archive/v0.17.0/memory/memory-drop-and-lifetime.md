...
| H12 | メモリ | 所有権/寿命チェックが構文パターンマッチのみで、変数経由のreturn・構造体フィールドへの格納・文字列補間経由のmove後使用ですり抜ける | 実装済み（本設計のスコープ=同一関数線形フロー内の主要すり抜け診断は完了: フィールド経由move・引数渡し（consume(move s)）・構造体リテラルのフィールド格納後の使用・文字列補間内のmove後使用をいずれもエラー診断する。use-after-moveメッセージはi18n化済み。MIR側でmoveされた変数のデストラクタ登録を解除しmoved-out変数の二重解放も修正。分岐・ループをまたぐ厳密なCFGフロー解析は本文書の設計方針どおり将来課題） |
