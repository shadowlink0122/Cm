...
### 第4段のformat系: 非対象と確定（設計判断）

- format系（native 143関数・wasm 134関数、共通115関数）の乖離を実測した結果、実乖離は85関数と大半を占め、その性質はslice系のような命名差でなくアーキテクチャ差だった: nativeはSDS文字列ヘッダ（cm_str_hdr/cm_str_alloc、最短round-trip dtoa）を前提とし、wasmは独自の簡易プリミティブ層（wasm_int_to_str/wasm_dtoa_prec等）の上に実装されている。HOF配列ビルトインもnativeのみCM_HOF_UNWRAP（負サイズ番兵によるスライスヘッダ展開）の呼び出し規約を持つ。
- 本文書の前提「format大半は挙動同一」は実測で否定されたため、format系の一本化は「wasm文字列基盤のSDS化」という別設計を前提とする将来課題として切り出し、本文書の対象からは外す。それまでの二重実装のシグネチャ乖離は第3段の検査（native/wasm相互照合191件）がlint/CIで防止する。

## 解決記録

7箇所同期問題への対応は次の形で確定した: (1)宣言=レジストリ表からの導出（第1段）、(2)js判定=レジストリ照合（第2段前半。写像if連鎖の表駆動化は各caseがjs固有意味論のため不採用）、(3)シグネチャ整合=lint/CIの突き合わせ検査（第3段。実乖離2件を検出・修正）、(4)slice系ランタイム=共通ソース一本化（第4段前半。二重実装1,082行削減・プラットフォーム差5フック化）、(5)format系ランタイム=アーキテクチャ差のため二重実装を維持し検査で防衛（本節）。
将来課題: wasm文字列基盤のSDS化とformat系一本化、js固有42件のレジストリ収容（宣言経路の優先順位整理が前提）、HOFのCM_HOF_UNWRAP規約のwasm側採否。
