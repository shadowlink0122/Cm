...
6. **既知の制約（R2連動）**: 非ASCIIコードポイントのデコードは[string-codepoint-byte-api-split.md](../strings/string-codepoint-byte-api-split.md)（R2）で解決済み——\u復号はfrom_bytes経由の全バックエンド共通経路になり、jsの非ASCII表示も一致する。
