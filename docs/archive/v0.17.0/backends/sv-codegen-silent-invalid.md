...
**ステータス:** 修正済み（表の#1〜#6を診断化。#7 native流入の専用診断はR14の領分として残置）
...
- **#1（sv::pinnタイポ黙殺）**: R7の属性検証レジストリは`sv::*`既知名を持っていたが、検証walkがGlobalVarDecl（SVポート宣言）を辿っておらず属性が未検査だった。walkへGlobalVarDeclを追加し、`#[sv::pinn]`等のタイポが未知属性警告（--strictエラー）になる。
- **#2（入力ポートへの代入）**: checkerへSVプラットフォームフラグ（`set_sv_platform`。ターゲット指定と`//! platform: sv`の両方で有効化）と`#[input]`ポート集合を追加し、プロセス内からの入力ポート代入をi18n診断で拒否する。`#[test]`テストベンチはDUTの外部として入力を駆動するため対象外（既存のtb系テストで検証）。
- **#3（エッジ無しalways_ffの黙殺）**: SV codegenの分岐順バグで、`is_always && !has_explicit_edge`の分岐がFF指定を飲み込み黙ってalways_combへ変換していた（下流のFF専用分岐は到達不能だった）。エッジ無しalways_ffは`error[SV008]`で停止する（順序回路の意図が黙って失われるのを防ぐ）。
- **#4（bit[0]）**: パーサの配列サフィックス解析で、Bit基底型のサイズ0を診断で拒否する。
- **#5（幅付きリテラルの桁あふれ）**: レクサのSV幅付きリテラル走査で`値 > 2^幅-1`を検査し、R5で整備したメッセージ付きErrorトークン経路（レクサ発行→パーサ診断化）で`4'd99`等を位置つきエラーにする。
- **#6（sv::pinの非文字列引数）**: パーサの属性引数解析で、`sv::pin`/`verilog::pin`の第1引数が数値リテラルなら専用診断（ピン名は文字列で指定）。引数なしはchecker側で診断。
- **#7（SV構文のnative流入診断）**: 修正方針どおりR14（構文エラー診断品質）の領分として残置。
- **テスト**: `tests/sv/errors/`へ5本（input_port_assign・ff_no_edge・bit_zero_width・literal_overflow・pin_numeric_arg）。SVスイート130件（新規5本含む）・全バックエンドスイートPASS。
