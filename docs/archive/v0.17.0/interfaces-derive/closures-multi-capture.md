# 複数変数キャプチャ対応クロージャ（クロージャ環境化）（実装済み）

## 対象所見

| # | 領域 | 所見 | 状態 |
|---|------|------|------|
| C6 | スライス/言語 | 2変数以上キャプチャするクロージャがnativeでLLVM検証エラー、jsではキャプチャを黙って落とし誤答 | 実装済み（ランタイムのクロージャ型をvoid*環境ポインタへ統一し、LLVMコード生成が呼び出し点でキャプチャ列をスタック上のi64環境配列へ格納し環境からキャプチャを復元してラムダを呼ぶサンクを合成する。js/tsは全キャプチャを正順で渡すよう修正。あわせてlong要素のmap/filterがi32版ランタイムへ落ちるディスパッチ欠落と、非クロージャ版map_i64/filter_i64のLLVM宣言欠落も修正） |
...
5. `reduce`/`sort`等クロージャ非対応の高階関数へのクロージャ対応拡張、またはクロージャ引数を型検査で明示拒否（黙殺排除）。（未対応の残課題。ただし現状は黙殺ではなく音を立てて失敗する: native/jit/wasmはreduce等のランタイム実体が存在せずリンク/宣言エラー、jsのreduce×クロージャはbind引数のローカル名不一致でReferenceErrorになる）
