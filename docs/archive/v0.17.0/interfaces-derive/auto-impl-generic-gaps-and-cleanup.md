...
- **診断から正常系への昇格（スライス）と是正**: スライス型引数のEq（`Box<int[]>`）は正常系へ昇格した（`tests/common/interface/derive/generic_slice_arg.cm`。等値・内容不等・長さ不等の3ケース）。昇格直後の実測で2件の実害を発見・修正した——(1)総称構造体リテラルのフィールド型が型パラメータ（T）のままでスライスフィールド判定に入らず、固定配列blobが生格納され不正CmSlice*になる（環境依存SIGSEGVの真因。lower_struct_literalへ型パラメータ置換を追加）。(2)総称本体の`self.v == other.v`はローワ時に型がTでHIRの配列脱糖対象外のため生MIR Eqのまま特殊化され、codegenのstrcmp catch-allへ落ちていた（LLVM Eq/Neへ動的スライスのcm_slice_equal分岐を追加し、モノモーフィゼーションへ特殊化後正準化`canonicalize_specialized_equality`＝ユニオン/スライスの生Eq/Ne書き換えパスを新設）。
- **ユニオン型引数のEqは診断を維持（設計判断）**: 宣言フィールドのユニオンEq（HIR脱糖が効く）は正常動作へ緩和したが（`tests/common/interface/derive/union_field.cm`）、ユニオンを「型引数」に渡す特殊化は、特殊化キーのユニオン正準エンコードとtypedef同一視が未整備で同一ユニオンがtypedef名（`Box__IU`）と表示形（`Box__int | string`＝識別子不正）の別特殊化に分裂するため、特殊化時診断を維持した。恒久解（typekeyの`$U`エンコード導入）は特殊化後正準化の一般機構化の設計文書で扱う。string/浮動小数型引数のHash等の未対応組合せも従来どおり特殊化時に診断される（G<string> with Hashの実測で確認）。
