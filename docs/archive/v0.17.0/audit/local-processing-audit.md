...
- ~~**A1 [reject] 文の宣言判定 `is_type_start()` が `typeof`/`(` を型開始と認識しない**（`src/internal/syntax/parser/parser_stmt.cpp:414`）。`typeof(x) y = ...;` と `(int) x = ...;` は宣言と認識されず式文として失敗するが、同じ型は仮引数・戻り値・フィールド・typedef・`as`（`parse_type`経由）では通る。`const (int) k` は `const` が宣言経路を起動するため通る、という位置依存の不整合。~~ → **処置済み**: typeof分はB系の処置で解消済み（typeof宣言の先読み）。paren分は `is_type_start` へ `(` の先読み分岐を追加した（括弧を閉じた直後——ポインタ/配列サフィックスを挟んで——に識別子が来る形は式として不成立のため宣言と確定できる。typeof宣言の先読みと同型）。`(int) k = 5;`・`(int | string) u = 42;`（A4のparen-union受理と連動）・`(int)* p = &k;` が全バックエンドで動作し、`(a) = 7;`（代入）・`(a + 2) * 3`（式）の誤宣言化が無いことを確認した。回帰テスト: `tests/common/types/general/paren_decl.cm`。
...
- ~~（補足）配列サイズ文法が `IntLiteral` か素の `Ident` のみを受け付け（`parser_type.cpp` `check_array_suffix`）、`int[2+1]`・`int[sizeof(int)]` 等の定数式が型位置に合成できない。~~ → **処置済み（算術定数式）**: `check_array_suffix` を拡張し、単純リテラル `T[3]`・constパラメータ名 `T[N]` 以外の内容は式としてパースし、整数リテラルの算術（`+ - * / % & | ^ <<` と単項 `- ~`）をパース時にコンパイル時定数へ畳む（`T[2+1]`・`T[(1+2)*3]`・`T[10/2]`）。パース時に畳むため局所・フィールド・仮引数の全型位置で `T[3]` と同一に動く（畳み後は素の固定長配列型で、checker/codegenに変更不要）。畳めない内容（`sizeof(int)`・const名の算術 `N+1` 等はパース時に値未知）は診断 `PsArraySizeNotConstant` で停止する（従来は `Expected ']'` の誤誘導）。回帰テスト: `tests/common/arrays/fixed/const_size_expr.cm`・`tests/common/errors/arrays-slices/const_size_nonconst.cm`。あわせて、この掃引で発覚した**構造体フィールドのconstパラメータ配列サイズ（`int[N] data`）が未解決になる別の穴**——`resolve_array_size` がローカル宣言（`check_let`）でのみ呼ばれ、フィールドは `size_param_name` が未解決のままでnative/jitのレイアウトが壊れ無出力になっていた——を、`check_struct` のフィールド検査で `resolve_array_size` を呼んで解消した（仮引数 `int[N] xs` は元から動作。回帰テスト: `tests/common/structs/field/const_array_size.cm`）。→ **処置済み（const名・sizeofを含む定数式）**: パース時に畳めない内容（const名の算術 `T[N+1]`・`T[N*2]`・`sizeof` を含む式）は、パーサが診断で止めずに式を `ast::Type::size_expr` へ保持し（`make_array_with_expr`）、型チェッカの `resolve_array_size` がコンパイル時評価 `evaluate_const_expr` で `array_size` へ畳む2段構成にした（const名はスコープが要るため解決を後段化。`evaluate_const_expr` へ `sizeof(型)` の評価——プリミティブ・ポインタ・その固定長配列のバイトサイズ——も追加）。畳み後は素の固定長配列型で後段は具体サイズのみ見るため、局所・フィールド・仮引数の全型位置で `T[3]` と同一に動く（全バックエンド一致を実測）。畳めない内容（実行時値・未定義const・構造体のsizeof=レイアウト計算がHIR層のため対象外）は従来どおり `PsArraySizeNotConstant` で停止する。回帰テスト: `tests/common/arrays/fixed/const_size_expr.cm`（const名・sizeof変種を追補）・`tests/common/errors/arrays-slices/const_size_nonconst.cm`（非定数例を実行時変数へ更新）。

**処置記録（A2）**: sizeof/`__sizeof__` の被演算子判定を共通ヘルパ `parse_sizeof_operand` へ一本化し（両組込に重複していた `could_be_type` スイッチを解消）、識別子始まりの被演算子は型パス（名前・`::`修飾・ジェネリック引数・ポインタ/配列サフィックス）が閉じ括弧まで到達する場合のみ型として解析する非破壊判定 `sizeof_operand_ident_is_type` を追加した。これにより `sizeof(p.x)`・`sizeof(f())`・`sizeof(x+y)` は式として解析され（従来はパースエラー）、型形の `sizeof(Point)`・`sizeof(Point*)`・`sizeof(Point[2])`・`sizeof(int[3])`・`__sizeof__(T)` は従来どおり型として解析される。識別子始まりの添字 `sizeof(a[0])` は型パスとして受理したうえで、型チェッカのsizeof救済を配列型へ拡張し、最内の被要素名が型でなく変数（配列/スライス）を指す場合は添字段数だけ要素型を剥がして要素型サイズを返すようにした（従来は要素数0の配列サイズ計算で無診断の0を返していた）。回帰テスト: `tests/common/types/general/sizeof_operand.cm`。残課題（当時。いずれも後続で処置済み）: A1（`typeof`/`(` を宣言開始と認識する `is_type_start` 拡張）と補足の配列サイズ定数式（`int[2+1]`）は本処置の対象外とした。

**処置記録（A3〜A6）**: A3は `as`/`is` の対象型の後置サフィックス消費を独自の空`[]`限定ラムダから正準の `check_array_suffix` へ置換し（`binary.cpp`）、`x as int[N]` が `(x as int)[N]` の添字式へ黙って化ける問題を解消した。配列型への `as` は実体loweringが無く無診断のゴミ値になるため、型チェッカで配列型キャストを専用診断 `TcCastToArrayUnsupported` で停止し、暗黙変換（代入・呼び出しの固定長→スライス）へ誘導する（ユニオン変種取り出し `union as int[]` は除外し既存のZ4検査へ委ねる）。あわせて `as` の `CastExpr` にスパンが設定されておらず新診断がstdlib先頭を指していた不具合を修正した（`is` 経路と同様に `Span{operand.start, end}` を付与）。A4は括弧型の中身を `parse_type` から union対応の `parse_type_with_union` へ再帰させ `(int | string)` をあらゆる型位置で受理（`parser_type.cpp`）、A5はジェネリック型引数ループを同じく `parse_type_with_union` へ変更し `Foo<int | string>` を受理（フィールド等でunionが通るのと対称化）した。A6は `__alignof__` の被演算子が型でなく変数を指す場合に静的型へ置換する変数救済を型チェッカへ追加し、未解決名の既定アライン8を返す無診断誤値を解消した（sizeofの変数救済と同型）。回帰テスト: `tests/common/types/general/type_grammar_union.cm`（A4/A5/A6）・`tests/common/types/casting/cast_to_array.cm`（A3の診断、`.error`）。A1もparen分の先読み追加で処置済みとなり、A系は全項目完了した（配列サイズ定数式はconst名・sizeof含め処置済み・`Type<Args>{...}` 構築式も処置済み。「その他の位置限定の非汎用」節参照）。
...
- **G2 [silent] JSが `x as typeof(intVar)` を解決しない**（native/wasmは切り詰める）。B4の裏返し。→ **処置済み**（B節の処置記録参照。`resolve_typeof` がキャスト対象型を具体型へ差し替えるためJSも全バックエンドと一致）。
- ~~**G3 [diag/silent] 集約の補間フォーマッタが未統一**。`int[3]` の `"{a}"` が interp/native `{}`・JS `1,2,3`、構造体は interp/native 空・JS `[object Object]`。共有規則が無く各backendが即興。~~ → **処置済み（診断による統一）**: 集約（非bit配列/スライス・登録済み構造体）の補間プレースホルダとprint/println直接引数を、型チェッカの共通ヘルパ `check_print_aggregate` が診断 `TcPrintAggregateUnsupported`（要素の個別出力またはdebug()等のstringを返すメソッドへ誘導）で停止するようにした。実測では直接引数 `println(arr)` はnative/jitで**LLVM検証失敗のクラッシュ**、スライス補間はnativeで**ゴミバイト出力**だったため、診断化は全分裂形の厳密な改善。canonicalフォーマットの導入（全バックエンド共通の整形規則+ランタイム実装）は需要が出た時点の将来課題とし、`{p.debug()}`（derive(Debug)）が全バックエンド一致の明示的整形手段として機能することを確認した。対象外: bitベクタ（整数として整形）・ユニオン（タグで実バリアントを整形するv0.16.0対応済み機能）・未登録名の構造体型（ジェネリック型パラメータT等、モノモーフ化前の見かけの型）。回帰テスト: `tests/common/errors/strings/interp_aggregate.cm`・`println_aggregate.cm`。
- **G4 [reject] 間接呼び出しのコード生成欠落**。`getf()(2,3)`（即時に返した関数ポインタの呼び出し）や `fs[0](2,3)`（関数ポインタ配列の呼び出し）がネイティブで `Symbols not found: [ _<indirect> ]`。型は通る。一旦変数に束ねれば動く。既知の関数ポインタ配列非対応（[array-literal-element-type-checking](../../archive/v0.17.0/arrays-slices/array-literal-element-type-checking.md) の対象外節）と同族。→ **一部処置済み**: HIRの呼び出しlowering（`expr.cpp`）が識別子以外の被呼び出し式に対して `indirect_callee` を設定していなかった（`func_name="<indirect>"` のまま→MIRが未解決シンボル `_<indirect>` になっていた）のを修正し、被呼び出し式を評価して間接呼び出しするようにした。これで**js/ts（構造的lowering）では `fs[0](args)`・`getf()(args)` が動作**する。native/jit/wasmは式値経由の間接呼び出しのコード生成（関数ポインタ配列の格納・値からの関数型導出）が未対応でランタイムクラッシュするため、checker（`function.cpp` の非識別子被呼び出し経路）で添字式・呼び出し結果の被呼び出しが関数ポインタ型のとき専用診断 `TcIndirectCallExprUnsupported`（「変数へ束ねてから呼び出す」誘導）で停止する（メンバフィールドの関数ポインタ呼び出し `h.f(args)` は別経路で全ターゲット動作するため対象外）。あわせて非識別子被呼び出しが無診断で `error` 型に落ちていた穴を、関数でない式の呼び出しは `TcNotFunction` 診断へ改めた。回帰テスト: `tests/common/functions/pointer/indirect_call_expr.cm`（js正常系）・`tests/common/errors/functions/indirect_call_expr_reject.cm`（native診断）。残: native/jit/wasm の式値間接呼び出しコード生成（別課題の深いcodegen作業）。
...
- 関数ポインタを直接呼ぶ式（G4）は一般に未対応（js/tsは処置済み・native系は診断化。G4参照）。~~`Type<Args>{...}` 構築式（`<`/`>`が比較演算子に解釈される）は一般に未対応。~~ → **処置済み**: パーサの `Ident<...>` 先読み（従来は `<...>::`・`<...>(` を処理）へ `<...>{` を追加し、`Box<int>{v: 7}` を `Box<int>` 名のstructリテラルとして解析するようにした（`Ident<バランスした型引数>{` は `a < b > {...}` が不正なため比較式と曖昧にならない）。型チェッカは明示型引数付き名（`Box<int>`）を基底名へ解決し、実際の型引数はフィールド値からの推論で決める（`Box{v: 7}` の推論経路と一致）。回帰テスト: `tests/common/generics/structs/explicit_type_args_literal.cm`。
...
3. **[High] A系: 型文法の呼び出し側複製を正準へ集約**。`is_type_start` を `parse_type` と整合させる（または投機パース化）、sizeof/alignof/as-is が同じ型パーサ経路を使う、paren・generic-arg を `parse_type_with_union` に。合わせてsizeof/alignofに変数救済を通す。→ **A2〜A6は処置済み**（A節の処置記録参照。A2=sizeof被演算子判定の共通化・変数被添字の要素型サイズ救済、A3=as/is配列サフィックスの正準化と配列as診断、A4=括弧型のunion受理、A5=ジェネリック引数のunion受理、A6=alignof変数救済）。A1も処置済み（typeof分=B系のtypeof宣言先読み・paren分=`(`の先読み分岐）となりA系は完了。
4. **[High] G1: JSの無指定補間に符号情報を渡し `asUintN` を適用**（silentな値破壊）。A2/A6/B1/B2のsizeof/typeof/alignof無診断誤値もここで一掃。→ **G1は解消済みを確認**（R20の補間統一で修正済み。G節参照。sizeof/typeof/alignofの無診断誤値もA/B系の処置で解消済み）
...
全5系統＋その他節の所見を処置し、本調査を完了した。処置の内訳:

- **修正（silent解消）**: C系（期待型伝播の透過ノード化）・E系（配列HOFの全幅ディスパッチ＋要素型ゲート）・D系（補間ホワイトリスト撤廃）・B系全項目（typeof被演算式の保持と解決）・A2/A3/A6/A補足（sizeof救済・as配列サフィックス正準化・alignof救済・配列サイズ定数式のconst名/sizeof対応）・G1/G2（確認/解消）・その他節全件（for-in string・generic fnptrフィールド・`Type<Args>{}`構築）。
- **受理の対称化（reject解消）**: A1（paren/typeof宣言）・A4/A5（union受理）・F系（縮小診断の全文脈化）。
- **診断による統一**: G3（集約のprint/補間はクラッシュ・ゴミ出力・バックエンド分裂だったため診断化し、debug()等の明示的整形へ誘導。canonicalフォーマット導入は需要が出た時点の将来課題）・G4のnative系（式値間接呼び出しはcodegen未対応のため`TcIndirectCallExprUnsupported`で診断化。js/tsは動作）。
- **残置（別課題として記録済みの拡張余地。いずれも診断で守られており無診断の誤りは無い）**: G4のnative/jit/wasm式値間接呼び出しコード生成（関数ポインタ配列の格納・値からの関数型導出を含む深いcodegen作業）・B3の仮引数typeof(変数)（登録時スコープ非在の極めて稀なケース。明確な型不一致エラーになる）・ラムダ仮引数の配列型構文。

調査の主眼だった「無診断の誤コンパイル/クラッシュ」は全て解消（修正または診断化）した。
