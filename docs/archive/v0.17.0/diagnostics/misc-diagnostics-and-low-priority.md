...
| L5 | 言語 | fmtはインデント正規化中心（演算子・波括弧の空白は不統一） | 実装済み（`normalize_operator_spacing`を曖昧性のない二項演算子（`=` 複合代入 `==` `!=` `<=` `>=` `&&` `\|\|` `=>` `<<=` `>>=`）へ拡張。既存空白は削らず不足分のみ追加するため意図的な整列を保持し冪等。単独の`+ - * / % < > & ^`は単項・ジェネリクス`<T>`・指数表記`1e+5`と曖昧なため対象外、`operator`宣言行は宣言スタイル`==(`を保持。リポジトリ全.cmの再フォーマット差分ゼロを確認） |
...
| L7 | 言語 | 並行処理はnativeバックエンド専用でチャネルはint64スカラのみ | 完了（制約はチュートリアル `docs/tutorials/{ja,en}/stdlib/concurrency/` に明文化済み: 対応バックエンドはNative (LLVM) のみ、チャネル搬送値は`long`でポインタはキャストして渡す。拡張はランタイム共通コア化と連動する将来項目） |
