...
v0.17.0で処置が完了した設計文書をサブシステム別カテゴリに整理したもの。全文書は「設計方針・段階分割・実装記録・不採用判断・将来課題」を記録している。変更の要約はリリースノート（[docs/releases/v0.17.0.md](../../releases/v0.17.0.md)）、調査系列の索引は調査系列の索引（[design-README.md](design-README.md)）を参照。
