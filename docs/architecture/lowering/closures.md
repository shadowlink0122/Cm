...
この二方式併存の設計判断と背景は [複数変数キャプチャ対応クロージャ（archive）](../../archive/v0.17.0/closures-multi-capture.md) に詳しい。
...
- 環境化はランタイムシグネチャをキャプチャ数非依存にするための設計であり、これが「2変数以上のキャプチャで宣言と実引数数が食い違いLLVM verifierが失敗する」バグのクラスを防いでいる（背景は [archive文書](../../archive/v0.17.0/closures-multi-capture.md)）。ランタイムの `_closure` 変種を追加するときは必ず `void* env` を第1引数で受ける形に揃えること。
...
- [複数変数キャプチャ対応クロージャ（設計文書、archive）](../../archive/v0.17.0/closures-multi-capture.md)
