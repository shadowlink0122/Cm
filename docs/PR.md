...
v0.14.0は**JavaScriptバックエンドの大規模改善**、**演算子オーバーロードの設計改善**、**ベアメタル/UEFIサポート**、**インラインユニオン型 (`int | null`)**、**プラットフォームディレクティブ**、**VSCode拡張機能の品質改善**を含むメジャーリリースです。JSテスト通過率が55%から85%に向上し、UEFIターゲットでのベアメタル開発が可能になりました。また、VSCode拡張機能をTypeScriptに移行し、ESLint/Prettierによるコード品質管理をCI統合しました。
