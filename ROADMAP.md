...
1. **implジェネリクスのLLVMコンパイル**
   - 問題: 関数パラメータの型置換が不完全
   - 影響: impl<T>メソッドがLLVMでコンパイルエラー
   - 回避策: インタプリタでは正常動作

2. **複数型パラメータとジェネリック関数**
   - 問題: ジェネリック関数が複数型パラメータ構造体を返す場合にGEPエラー
   - 影響: `<T, U> Pair<T, U> make_pair(T, U)`のような関数
   - 回避策: 構造体の直接操作は動作

3. **型制約チェック**
   - 問題: パースは可能だが実行時検証なし
   - 影響: `<T: Ord>`のような制約が検証されない
   - 回避策: 手動で型の適合性を確認
