...
|-----------|---------|----------|--------|------|
| 定数畳み込み (Constant Folding) | ⚠️ 部分実装 | [設計](docs/optimization/mir_optimization_roadmap.md) | インポート時無限ループ | 外部関数処理に問題 |
| デッドコード除去 (DCE) | ⚠️ 部分実装 | [設計](docs/optimization/mir_optimization_roadmap.md) | インポート時無限ループ | 外部関数処理に問題 |
| 定数伝播 (SCCP) | ✅ 実装済み | - | なし | |
| コピー伝播 | ✅ 実装済み | - | なし | |
| 共通部分式除去 (CSE/GVN) | ✅ 実装済み | - | なし | Local GVN実装 |
| デッドストア除去 (DSE) | ✅ 実装済み | - | なし | |
| 制御フロー簡約化 | ✅ 実装済み | - | なし | |
| インライン化 | ⚠️ 制限付き | [設計](docs/optimization/infinite_loop_detection.md) | 過度のインライン化 | 回数制限実装済み |
...
| 循環検出 | ⚠️ 部分実装 | ハッシュベース検出（履歴8個） |
| インライン化制限 | ✅ 実装済み | 関数ごと2回、全体20回まで |
| タイムアウト | ⬜ 未実装 | コンパイル時間制限 |
