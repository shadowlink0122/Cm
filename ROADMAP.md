...
#### 🟡 次期: モジュールシステム実装（Phase 1）
- [ ] `export { M };` 構文のパーサー実装
- [ ] 深い階層インポート（`import ./io/file;`）
- [ ] プリプロセッサでの再エクスポート解決
- [ ] 階層的インポート（`import std::io;`）の処理
- [ ] `namespace` 生成
- 設計文書: `docs/design/MODULE_SYSTEM_FINAL.md`

### 現在の状況
- **文字列補間の改善**: 🔄 部分完了（2024年12月20日）
  - ローカルconst変数の補間: ✅ 動作
  - グローバルconst変数の補間: ❌ 未実装
- **型システム**: ⚠️ 型チェックに問題あり
  - 関数戻り値の型不一致が検出されない
...
| export定数（単一ファイル） | ✅ | ✅ | ✅ | ✅ | ⬜ | ⬜ | ✅ |
| export構造体（パースのみ） | ✅ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| exportインターフェース（パースのみ） | ✅ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| export enum（パースのみ） | ✅ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| exportキーワード削除処理 | ✅ | ✅ | ✅ | ✅ | ⬜ | ⬜ | ✅ |
| main関数エクスポートエラー検出 | ✅ | ✅ | ✅ | ✅ | ⬜ | ⬜ | ✅ |

#### 部分実装の機能（パーサー/プリプロセッサは存在するが動作不完全）
| 機能 | パーサー | 型チェック | HIR/MIR | インタプリタ | LLVM | WASM | テスト |
|------|----------|-----------|---------|-------------|------|------|------|
| import文（基本） | ✅ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| モジュール解決 | ✅ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
...
注: 構造体メンバとして配列やポインタを持つ機能は、パーサーエラーにより現在サポートされていない。
