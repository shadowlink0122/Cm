...
1. `use`構文のパース（週1）
2. `cstring`型の追加（週1）  
3. extern "C"関数宣言のlowering（週1）
4. 基本的なFFI呼び出しテスト（週1）
5. `#[repr(C)]`属性（週2）
6. 可変長引数（varargs）- printf対応（週2）
...
注: インラインアセンブリはFFI完了後に実装予定。現時点ではFFIを優先。

### FFI実装（v0.10.0 Phase 4）

#### 基本構文の状況確認（2025-12-21）
| 基本機能 | 状態 | 備考 |
|----------|------|------|
| ポインタ型 `*T` | ✅ | 完全動作 |
| 配列型 `T[N]` | ✅ | 完全動作 |
...
#### 実装優先度
1. **usize/isize型**（1日）- longのエイリアスとして実装
2. **cstring型**（1日）- `*char`の特殊型として実装  
3. **use libc {...}構文**（3日）- パーサー拡張
4. **FFI関数呼び出し**（3日）- LLVM extern "C"連携
5. **varargs**（2日）- printf等で必要
6. **#[repr(C)]**（1日）- 構造体レイアウト制御
...
## Version 0.14.0 - インラインアセンブリとベアメタル拡張
