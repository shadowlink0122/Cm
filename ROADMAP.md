...
## Version 0.6.0 - ジェネリクス（現在の目標） 🚧

### 目標
型パラメータとジェネリックプログラミング（配列・Result型の前提条件）

### 実装項目
| 機能 | パーサー | 型チェック | HIR/MIR | LLVM |
|------|----------|------------|---------|------|
| 関数ジェネリクス構文 | ✅ | ⬜ | ⬜ | ⬜ |
| 構造体ジェネリクス構文 | ✅ | ⬜ | ⬜ | ⬜ |
| インターフェースジェネリクス構文 | ✅ | ⬜ | ⬜ | ⬜ |
...
### 完了した作業（2024年12月）
- ✅ パーサーレベルでのジェネリック構文サポート
  - `<T> T max(T a, T b)` 形式の関数
  - `struct Vec<T>` 形式の構造体
  - `interface Container<T>` 形式のインターフェース
  - `impl Vec<T> for Container<T>` 形式のimpl（`impl<T>`は冗長なので不使用）
  - 型引数の解析（`Vec<int>`, `Map<K, V>`）
- ✅ **LLVM実行時サポート基盤**（2024年12月12日）
  - 基本的なLLVM IR生成とネイティブバイナリ生成
  - MIR生成の簡略化版実装（LoweringContextの問題を回避）
  - return文のリテラル値処理

### 現在の制限事項と既知の問題（2024年12月12日更新）

#### 重大な問題
- **論理演算の不具合**: AND/OR/NOT演算が正しく動作しない
  - SSA形式でのPhi node未実装により値の伝播が不正確
  - `true && false` が true を返すなどの根本的問題
- **break/continue文**: 論理演算の問題により無限ループが発生
- **文字列フォーマット**: プレースホルダー `{}` が値で置換されない

#### 制限事項
- MIRローダリングは簡略化版（LoweringContextに問題があるため直接構築方式）
- 関数本体処理は部分的（return文のリテラル値のみ）
- 式評価や変数処理は未実装

### 残作業と解決計画

#### 緊急対応（論理演算の修正）
- **Phase 1**: LLVMレベルでPhi nodeを直接生成（暫定対応）
- **Phase 2**: MIR層にPhi命令を追加し、SSA形式を正しく実装
- **Phase 3**: LoweringContextを拡張し、値の伝播を改善
- 詳細は `docs/design/mir_ssa_redesign.md` 参照

#### ジェネリクス関連
- 型チェッカーでのジェネリック型認識
- HIR/MIRでのジェネリック変換
- 完全なMIRローダリング実装
- LLVMコード生成でのモノモーフィゼーション
...
## Version 0.7.0 - エラーハンドリングとResult型

### 目標
安全なエラーハンドリングの実装（v0.6.0のジェネリクスが前提）
...
## 次のアクション（v0.6.0ジェネリクス完成に向けて）

1. **型チェッカーの拡張** - 1週間
   - ジェネリック型パラメータの認識（T, U, K, V等）
   - 型置換メカニズムの実装
   - ジェネリック関数の型チェック
